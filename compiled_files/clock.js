...
function moveSecondsHand(hand) {
    ...
    hand.style.transform = "translateX(-50%) rotate(".concat(secondAngle, "deg)");
}
function moveMinutesHand(hand) {
    minuteAngle += 6; // 360 / 60
    hand.style.transform = "translateX(-50%) rotate(".concat(minuteAngle, "deg)");
}
function moveHoursHand(hand) {
    hourAngle += 6; // 360 / 12 (but here it jumps fast, just for demo)
    hand.style.transform = "translateX(-50%) rotate(".concat(hourAngle, "deg)");
}
// Run with intervals
setInterval(function () { return moveSecondsHand(secondHand); }, 1 * 1000);
setInterval(function () { return moveMinutesHand(minuteHand); }, 60 * 1000);
setInterval(function () { return moveHoursHand(hourHand); }, 12 * 60 * 1000);
